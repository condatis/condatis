...
########## ACCESS ###############################################
    ...
    def sigPower(self):
        if self.scenario.__contains__("sig_power"):
            return self.scenario.sig_power.read()
        else:
            return 0
...
#            print "Got total link strength, it's ",self.scenario._v_attrs.totalLinkStrength
    ...
    def combinedSize(self):
        x,y=self.combinedHabitat()
        xz=int(np.max(x)-np.min(x)+2)
        yz=int(np.max(y)-np.min(y)+2)
        return xz,yz
...
########## CROSS SCENARIO #########################################
    def allFlows(self):
        ...
        for i in range(N):
            ...
            r[i]=con.getNth(self.h5file,i)._v_attrs.I0
        return r
